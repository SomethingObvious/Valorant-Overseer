...
def _summary(points: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    # A drawn match is neither, and counting it as neither made the session
    # read as though it had never happened.
    draws = sum(p.get("result") == "Draw" for p in rated)
    ...
    return {
        ...
        "losses": losses,
        "draws": draws,
        # Out of the matches that could be won. A draw is not a loss, so it
...
def recap_of_last_match(lm: Any) -> dict[str, Any] | None:
    """The last match played, rather than the last one watched.

    `current_recap` only knows about a match that finished while the app was
    running, and forgets it after ten minutes. That is the right thing for the
    ...
    try:
        ...
        return None
    last = history_list[0] if history_list else None
    match_id = (last or {}).get("matchId")
    if not match_id:
        return None
    with _LOCK:
        cached = _STATE.get("last_match_recap")
        if isinstance(cached, dict) and cached.get("matchId") == match_id:
            return cached
    recap = _build_recap(lm, {"matchId": match_id, "players": [], "mode": (last or {}).get("mode")})
    if recap:
        with _LOCK:
