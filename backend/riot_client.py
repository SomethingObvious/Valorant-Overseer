...
from vconstants import GAMEMODES, ROUTING, map_name_from_path

# Only the loopback calls below pass verify=False (the Riot client ships a
# self-signed cert), so this suppresses warnings for those and nothing else.
# Every remote *.a.pvp.net call verifies normally -- they carry the
# entitlements and access tokens, and used to cross the network unverified.
...
class LocalAuth:
    ...
    @staticmethod
    def available() -> bool:
        path = str(Path(os.getenv("LOCALAPPDATA", "")) / r"Riot Games\Riot Client\Config\lockfile")
    ...
    def _get_region(self) -> list[Any]:
        path = str(Path(os.getenv("LOCALAPPDATA", "")) / r"VALORANT\Saved\Logs\ShooterGame.log")
        pd_url = glz_url = None
    ...
    def glz_get(self, endpoint: str) -> dict[str, Any]:
        _riot_throttle()
        self.req_count += 1
        return self._json(requests.get(self.glz_url + endpoint, headers=self.headers(), timeout=8))
    ...
    def pd_get(self, endpoint: str, refresh: bool = False, retries: int = 0) -> dict[str, Any]:
        ...
        for attempt in range(retries + 1):
            ...
            self.req_count += 1
            resp = requests.get(self.pd_url + endpoint, headers=self.headers(refresh), timeout=8)
            if resp.status_code == 429:
                try:
...
class RiotClient:
    ...
    def _official_riot_id(self, puuid: str) -> str | None:
        cluster = ROUTING.get(self.region, "americas")
        try:
            r = requests.get(
