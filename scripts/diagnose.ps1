    ...
    foreach ($c in $conns) {
        ...
        try {
            ...
            if ($exe) {
                ...
                if ($exeLower -eq $rootLower -or $exeLower.StartsWith($rootLower + '\')) { return "ours (Valorant Overseer, PID $portPid)" }
            }
            if ($proc.CommandLine) {
                $cmdLower = $proc.CommandLine.ToLower()
                if ($cmdLower -eq $rootLower -or $cmdLower.Contains($rootLower + '\')) { return "ours (Valorant Overseer, PID $portPid)" }
            }
...
if (Test-Path $lockfile) {
    try {
        ...
        Bad "VG-RIOT-001 Riot lockfile exists but can't be read - try restarting the Riot Client"
    }
}
else {
