...
$AllowExact = @(
    ".gitattributes", ".gitignore", "README.md", "VERSION",
    "runtime.json", "install.bat", "start.bat",
    "run.py"
)
# tui/ ships the prebuilt scoreboard bundle plus its TypeScript sources.
# The bundle is what runs -- users need node and nothing else, no npm
# install -- and the sources ship because a bundle nobody can read is
# exactly the opacity this build exists to avoid. node_modules is
# gitignored, so it is never in the payload to begin with.
$AllowPrefix = @("assets/", "backend/", "docs/", "tui/")
...
if ($payload.Count -lt 40) { Stop-Build "suspiciously small payload ($($payload.Count) files) - allowlist broken?" }
...
try {
    ...
    foreach ($req in @("run.py", "start.bat", "install.bat",
            "runtime.json", "backend\app.py", "backend\requirements.txt",
            "scripts\common.ps1", "scripts\start.ps1")) {
