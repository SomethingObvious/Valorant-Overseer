...
function Compare-OverseerVersion([string]$a, [string]$b) {
    $a = ($a.Trim()) -replace '^[vV]', ''
    $b = ($b.Trim()) -replace '^[vV]', ''
...
function Stop-RunningApp([string]$LogName = "launcher") {
    ...
    Note "Closing the running Valorant Overseer (PID $appPid) so we can continue ..."
...
function Test-Preflight {
    ...
    if ($Root -match '[\[\]]') {
        $problems += "The folder name contains square brackets [ ]: '$Root'. Rename the folder to remove them (e.g. 'overseer [1]' -> 'overseer'), then run install.bat again."
    }
    ...
    foreach ($dir in @($Root, $env:TEMP)) {
        try {
            ...
            $problems += "The folder '$dir' is not writable. Move Valorant Overseer to a folder you can write to (e.g. Documents)."
        }
    }
...
function Initialize-ExactPython {
    ...
    if (-not $py) {
        throw "VG-PY-001 CPython $($mf.python.version) 64-bit could not be found or installed. Install it manually from $($mf.python.installerUrl) and run install.bat again."
    }
...
function Install-PyDeps {
    ...
    & $VenvPy -m pip check
    if ($LASTEXITCODE -ne 0) { throw "VG-DEPS-001 pip check failed after install, the dependency set is inconsistent." }
    & $VenvPy (Join-Path $PSScriptRoot "verify_installed.py") `
...
function New-DesktopShortcut {
    ...
    try {
        ...
        # Not "exists, so leave it": a shortcut left over from an install at a
        # different path points at a folder that may no longer be there, and
        # silently launches nothing. Repoint it instead.
        if (Test-Path $lnk) {
            $cur = $ws.CreateShortcut($lnk)
            if ($cur.TargetPath -eq $target) { Note "Desktop shortcut already points here."; return }
            Note "Desktop shortcut pointed at $($cur.TargetPath) - repointing it."
        }
        ...
        $sc.Description = "Launch Valorant Overseer"
        $sc.Save()
